...
time_windows = [2 : 31]; 
Ke = X{    4   }; % 316 316 219 31
ref_img = nanmean(  Ke(:, :, 4, time_windows), 4); % 316 316

%%
trans_att_in = cell(9, 2); 
for n = 1 : 9
    time_windows = [2 : 31];
    Ke = X{    n   }; % 316 316 139 31
    I = nanmean(Ke(:, :, :, time_windows), 4); % 316 316 139
    
    % imwarp(b, best_tform, 'OutputView', imref2d(size(a)));
    % I_aligned - 316x316x207
    % ref_aligned - 207x1
    [I_aligned, transform_info] = align_ImgSet(I, ref_img); 
    
    % Img = nanmean(I_aligned, 3);
    % figure, imagesc(Img)
    trans_att_in{   n, 1  } = transform_info;
...
for n = 10 : 18
    ...
    % Img = nanmean(I_aligned, 3);
    % figure, imagesc(Img)
    trans_att_out{   n - 9, 1  } = transform_info;
